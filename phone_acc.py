...
def calculate_breathing_rate(normalized_data):
    smooth_data = apply_average_filter(
        normalized_data, average_filter_window_duration_br
    )
    plot(
        smooth_data[:, 0],
        "Smoothened Accelerometer Data",
        "plots/bio_watch/smoothened_ax.png",
    )
    ...
    br_x_amp, br_x_f = fft(
        ...
        breathing_low_freq,
        breathing_high_freq,
        "plots/bio_watch/br_fft_xaxis.png",
    )
    ...
    br_y_amp, br_y_f = fft(
        ...
        breathing_low_freq,
        breathing_high_freq,
        "plots/bio_watch/br_fft_yaxis.png",
    )
    ...
    br_z_amp, br_z_f = fft(
        ...
        breathing_low_freq,
        breathing_high_freq,
        "plots/bio_watch/br_fft_zaxis.png",
    )
...
def calculate_heart_rate(normalized_data):
    smooth_data = apply_average_filter(
        normalized_data, average_filter_window_duration_hr
    )
    smooth_data = np.array(
        list(filter(lambda row: np.isfinite(np.sum(row)), smooth_data)),
        dtype=np.float64,
    )
    plot(
        smooth_data[:, 0],
        "Smoothened Accelerometer Data - HR",
        "plots/bio_watch/smoothened_ax_hr.png",
    )

    low_cutoff_freq = 4
    high_cutoff_freq = 11
    smooth_data[:, 0] = apply_bandpass_butterworth_filter(
        smooth_data[:, 0], low_cutoff_freq, high_cutoff_freq
    )
    smooth_data[:, 1] = apply_bandpass_butterworth_filter(
        smooth_data[:, 1], low_cutoff_freq, high_cutoff_freq
    )
    smooth_data[:, 2] = apply_bandpass_butterworth_filter(
        smooth_data[:, 2], low_cutoff_freq, high_cutoff_freq
    )
    plot(
        smooth_data[:, 0],
        "Bandpass-1 Accelerometer Data",
        "plots/bio_watch/bandpass1_ax.png",
    )
    ...
    bandpass2_data = apply_bandpass_butterworth_filter(
        aggregated_data, low_cutoff_freq, high_cutoff_freq
    )
    plot(
        bandpass2_data,
        "Pulse wave from Accelerometer Data",
        "plots/bio_watch/pulse_wave.png",
    )
...
def bio_watch(data, sampling_freq):
    ...
    plot(
        ...
        "plots/bio_watch/normalized.png",
    )
...
input_file_path = "accelerometer_data_from_phone/merged_27-03-2025.txt"
